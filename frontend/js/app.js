...
// --- Global Supabase Session State ---
// Populated once on DOMContentLoaded by SupabaseSession.init()
window._ohiseeUser = null;  // Full profile from public.users table
window._ohiseeSession = null; // Supabase auth session
...
const SupabaseSession = {
  ...
  async requireAuth(allowedRoles = null) {
    await this.init();
    if (!window._ohiseeUser) {
      window.location.href = 'login.html';
      return false;
    }
    if (allowedRoles && !allowedRoles.includes(window._ohiseeUser.role)) {
      window.location.href = 'login.html';
      return false;
    }
    return true;
  ...
  // Logout: sign out from Supabase + clear state
  async logout() {
...
// --- Product Catalog Synchronization (Supabase-backed with localStorage fallback) ---
(function syncProducts() {
  // If Supabase cache is loaded later via event, update PRODUCTS
  window.addEventListener('supabaseProductsLoaded', (e) => {
    if (e.detail && Array.isArray(e.detail.products) && e.detail.products.length > 0) {
  ...
  // Fallback: keep static PRODUCTS array for pages that need it before Supabase loads
...
const PartnerManager = {
  ...
  getPartnerPrice(productPrice, user) {
    const isApproved = user && (user.partnerStatus === 'approved' || user.partner_status === 'approved');
    if (!user || user.role !== 'Partner' || !isApproved) {
      return productPrice;
    }
    const discount = this.getDiscountPercentage(user.partnerTier || user.partner_tier || 'Silver');
...
// --- RBAC & Session Management (Supabase-backed) ---
const Security = {
...
// --- Cart Management (Supabase-backed with localStorage fallback) ---
const Cart = {
  KEY: 'ohisee_cart',
  ...
  async addItem(productId, qty = 1) {
    ...
    if (user) {
      try {
        await SupabaseCart.incrementItem(user.id, productId, qty);
      } catch (e) {
  ...
  async removeItem(productId) {
    ...
    if (user) {
      try {
        await SupabaseCart.removeItem(user.id, productId);
      } catch (e) {
        console.error('Supabase cart sync error:', e);
      }
    }
  ...
  async updateQty(productId, qty) {
    ...
    if (user) {
      try {
        await SupabaseCart.updateQty(user.id, productId, qty);
      } catch (e) {
        console.error('Supabase cart sync error:', e);
      }
    }
  ...
  async clear() {
    ...
    if (user) {
      try {
        await SupabaseCart.clearCart(user.id);
      } catch (e) {
        console.error('Supabase cart sync error:', e);
      }
    }
  ...
  async syncFromSupabase() {
    ...
    if (!user) return;
    try {
      const dbItems = await SupabaseCart.fetchItems(user.id);
      ...
      console.warn('Failed to sync cart from Supabase:', e.message);
    }
  },
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Initialize Supabase session (non-blocking)
  await SupabaseSession.init();

  // Sync cart from Supabase (non-blocking)
  ...
  // Update header account link based on role
  const user = Security.getCurrentUser();
  if (user) {
    const role = user.role;
    document.querySelectorAll('a[href="account.html"]').forEach(a => {
      if (role === 'Admin' || role === 'Super Admin') a.href = 'admin.html';
      else if (role === 'Partner') a.href = 'partner.html';
    });
  }
